...
class GA:
    def __init__(self,parameter):
        ...
        self.cost1=self.parameter['cost1']
        self.cost2=self.parameter['cost2']
        self.total_need=self.parameter['need']
        self.left=self.parameter['left']
        ...
        for i in range(self.parameter['popsize']):
            ...
            #评估基因适应性
            fitness=self.evaluate(geneinfo,self.cost1,self.cost2,self.total_need,self.left)
            #加入种群
    ...
    def evaluate(self,x,cost1,cost2,total,left):
        #x:订购量，cost1：购买花费，cost2：转运花费    
        tmp=left+np.sum(x)-total
        return -(np.sum(np.dot(cost1,x))+np.sum(x)*cost2-tmp/total)
